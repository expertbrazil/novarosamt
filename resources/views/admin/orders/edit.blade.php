...
<div class="space-y-6">
                ...
                <ol role="list" class="flex items-center space-x-4">
                    ...
                    <li>
                        <div class="flex items-center">
                            ...
                            <a href="{{ route('admin.orders.show', $order) }}" class="ml-4 text-sm font-medium text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-300">
                                #{{ $order->id }}
                            </a>
                        </div>
                    </li>
    ...
    <!-- Form -->
    <form method="POST" action="{{ route('admin.orders.update', $order) }}" id="adminOrderForm" class="space-y-6">
        @csrf
        @method('PUT')
                        ...
                        <div class="mt-1">
                            <select name="customer_id" 
                                ...
                                @foreach($customers as $customer)
                                    <option value="{{ $customer->id }}" {{ (int)(old('customer_id', $orderCustomer?->id)) === $customer->id ? 'selected' : '' }}>
                    ...
                    <div>
                        ...
                        <div class="mt-1">
                            <select name="payment_method" 
                                ...
                                <option value="credito" {{ old('payment_method', $order->payment_method)=='credito' ? 'selected' : '' }}>Cartão de Crédito</option>
                                <option value="debito" {{ old('payment_method', $order->payment_method)=='debito' ? 'selected' : '' }}>Cartão de Débito</option>
                                <option value="dinheiro" {{ old('payment_method', $order->payment_method)=='dinheiro' ? 'selected' : '' }}>Dinheiro</option>
                                <option value="boleto" {{ old('payment_method', $order->payment_method)=='boleto' ? 'selected' : '' }}>Boleto</option>
                                <option value="transferencia" {{ old('payment_method', $order->payment_method)=='transferencia' ? 'selected' : '' }}>Transferência</option>
                            </select>
                        </div>
                ...
                <div>
                    ...
                    <div class="mt-1">
                        <textarea name="observations" 
                                  ...
                                  placeholder="Adicione observações sobre o pedido, entrega, pagamento, etc...">{{ old('observations', $order->observations) }}</textarea>
                    </div>
                    @error('observations')
                        <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                    @enderror
